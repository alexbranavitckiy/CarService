...
public class CRUDServicesClientImpl implements CRUDServices, SearchServices<Client> {
  ...
  public List<Client> getAll(File file) throws EmptySearchException {
    try {
      return new ArrayList<>(List.of(ObjectMapperServices.getObjectMapper()
          .readValue(new File(FileService.USER_PATH), Client[].class)));
    } catch (IOException e) {
  ...
  public boolean addObject(Object o, File file) {
    ...
    try {
      List<Client> list = new ArrayList<>(Arrays.asList(ObjectMapperServices
          .getObjectMapper().readValue(file, Client[].class)));
      list.add(client);
  ...
  public boolean deleteObjectById(String id, File file) {
    try {
      List<Client> clients = new ArrayList<>(Arrays.asList(ObjectMapperServices.getObjectMapper()
          .readValue(file, Client[].class)));
  ...
  public boolean updateObject(Object o, String id, File file) {
    if (this.deleteObjectById(id, new File(FileService.USER_PATH)) && this.addObject(o,
