...
public class CRUDServicesMasterImpl implements CRUDServices {
    ...
    public boolean addObject(Object o, File file) {
        ...
        try {
            List<Master> list = new ArrayList<>(Arrays.asList(ObjectMapperServices.getObjectMapper().readValue(file, Master[].class)));
            list.add(client);
    ...
    public boolean deleteObjectById(String id, File file) {
        try {
            List<Master> clients = new ArrayList<>(Arrays.asList(ObjectMapperServices.getObjectMapper().readValue(file, Master[].class)));
