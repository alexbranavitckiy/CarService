...
public interface OutfitsRepository extends PagingAndSortingRepository<Outfit, UUID> {
 ...
 @Transactional
 @Modifying
 @Query(value = "UPDATE public.outfit SET state_outfit =?1  where outfit.id = ?2 and outfit.id_master in( select master.id from master  where master.login=?3)", nativeQuery = true)
 int startWorkMaster(String work, UUID uuid, String login);

 @Transactional
 @Modifying
 @Query(value = "UPDATE car_breakdown  set state='IMPORTANT'  where  state='IMPORTANT' and  id_outfit in( SELECT id FROM outfit where  state_outfit='WORK' and id_master in( select id from master  where login=:login));", nativeQuery = true)
 int checkingOutfit(String login);

 @Transactional
 @Modifying
 @Query(value = "UPDATE public.outfit SET state_outfit =?1  where  outfit.state_outfit='WORK'  and outfit.id_master in( select master.id from master  where master.login=?2)", nativeQuery = true)
 ...
 @Query(value = "SELECT * FROM public.outfit where date_end between :start and :end and date_start  between  :start and :end  and id_master = :master", nativeQuery = true)
