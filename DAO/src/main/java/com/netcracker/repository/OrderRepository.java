...
public interface OrderRepository extends JpaRepository<Order, UUID> {
 ...
 @Query(value = " SELECT orders.id, created_date, orders.description, state, updated_date, id_car, id_outfits" +
  " FROM public.orders as orders  left join public.clients c  on c.login =?1 and orders.state =?2", nativeQuery = true)
 List<Order> getAllOrderClient(String login, String state);

 @Transactional
 @Modifying
 @Query(value = "UPDATE public.orders   SET state =?1,updated_date=?2" +
  "  where orders.id = ?3 and orders.id_car" +
  " in( select car_client.id from car_client where car_client.id_clients  in(select clients.id from clients where clients.login=?4))", nativeQuery = true)
